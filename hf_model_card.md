...
> This preliminary exploratory suite sampled 10 items per task ($N=200$ macro) and is archived in [`eval_results/archive_deprecated/qwen35_2b_authentic_20_benchmarks.json`](https://huggingface.co/CH3NDev/dual-loop-qwen3.5-2b/blob/main/eval_results/archive_deprecated/qwen35_2b_authentic_20_benchmarks.json). Due to the small sample per task, individual task confidence intervals are wide ($\pm 7\text{--}8\%$). For instance, on **BBH-LogicalDeduction**, this 10-item snapshot logged $90.0\% \to 90.0\%$ ($\Delta = 0.0\%$), whereas our dedicated large-sample audit ($N=50$ item subset of $N=200$ audit, McNemar $p = 0.375$ [statistically non-significant] in [`eval_results/qwen35_2b_multistep_n200_eval.json`](https://huggingface.co/CH3NDev/dual-loop-qwen3.5-2b/blob/main/eval_results/qwen35_2b_multistep_n200_eval.json)) establishes true base performance at $68.0\%$, dropping to $60\text{--}62\%$ under uncalibrated deliberation. For the official multi-system comparative benchmark, refer to the **Official Leaderboard** above.
